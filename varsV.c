 ...
 */
static int _check_changed_field( fp, vars)
FILE *fp;
Variable_t *vars;
{  int N;
 ...
 */
		...
		if( c_v_PARENT_check ){
			if( vars->parent && &(((Variable_t*)vars->parent->var.ptr)[vars->id])!= vars ){
 ...
 */
RET_SHORT Sprint_var( fp, var, verbose_print)
Sinc *fp;
Variable_t *var;
int verbose_print;
...
RET_SHORT print_var( fp, var, addr)
FILE *fp;
Variable_t *var;
int addr;
{	Sinc sinc;
					  ...
					  int subst= 0, skip_rest= 0, prune= True;
						do{
							if( *p== REFERENCE_OP && p[1]== LEFT_BRACE ){
							  ...
							  char *cIndex= NULL, *end= &p[2], pend, *c= NULL, pc= 0;
							  Variable_t *rvar;
							  VariableSelection *vs;
							  int escaped= False;
								p+= 2;
								...
								*end= '\0';
								...
								else if( ( ((char*) rvar)= getenv(p) ) ){
									strncpy( rv, (char*) rvar, VARSEDIT_LENGTH-strlen(ref_val) );
...
{ Variable_t *vars;
  ...
  int i, j= 0, check_slash= 1, depth, exit_command= 0, nlines= 0, tlines= 0;
  long changes;
